...
export const owner = 'veloxpack';
export const DocsCategory = 'Docs Feedback';

let instance: Octokit | undefined;

async function getOctokit(repo: string): Promise<Octokit> {
...
async function getFeedbackDestination(repo: string) {
...
export async function onRateAction(
  ...
  'use server';
  const octokit = await getOctokit(repo);
  const destination = await getFeedbackDestination(repo);
