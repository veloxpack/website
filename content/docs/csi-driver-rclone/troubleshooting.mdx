...
# Check pod status
kubectl describe pod -n kube-system -l app=csi-rclone-controller
kubectl describe pod -n kube-system -l app=csi-rclone-node
```
...
kubectl exec -n kube-system -l app=csi-rclone-node -- ls /dev/fuse

# Check resource limits
kubectl describe pod -n kube-system -l app=csi-rclone-controller

# Check image pull
kubectl describe pod -n kube-system -l app=csi-rclone-controller | grep -i image
```
...
# Test configuration
kubectl exec -n kube-system -l app=csi-rclone-node -- sh -c 'echo "[s3]
type = s3
provider = AWS
...
kubectl top pods -n kube-system -l app=csi-rclone-node
...
kubectl exec -n kube-system -l app=csi-rclone-node -- nslookup s3.amazonaws.com
...
# Follow logs
kubectl logs -n kube-system -l app=csi-rclone-node -f

# Previous container logs
kubectl logs -n kube-system -l app=csi-rclone-node --previous
```
...
# Check mount options
kubectl exec -n kube-system -l app=csi-rclone-node -- cat /proc/mounts | grep rclone

# Check FUSE mounts
kubectl exec -n kube-system -l app=csi-rclone-node -- ls -la /dev/fuse
```
...
kubectl exec -n kube-system -l app=csi-rclone-node -- umount -f /var/lib/kubelet/pods/*/volumes/kubernetes.io~csi/*/mount
...
kubectl logs -n kube-system -l app=csi-rclone-controller > controller.log
kubectl logs -n kube-system -l app=csi-rclone-node > node.log
